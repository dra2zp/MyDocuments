...
(* Sullivan: Test-Driven Development! *)
...
(* 
  Sullivan: Types define sets of logical terms. It's important to understand
  that we then implicitly imbue these terms with real-world meanings. Thus,
  for example, we interpret the logical term, [O], as representing (meaning) 
  the natural  number, zero, and (S n) as meaning one more than n. 

  We then define functions that take and return such terms, and we then
  interpret these functions as having corresponding real world meanings.
  We interpret S to mean "one more than", and, in a moment, we will define
  other functions that operate on these terms, simulating real-world functions
  such as predecessor, natural number addition, multiplication, and so on.
  Logical terms of operations simulate real world things and operations on 
  them. A key requirement is that the logical operations behave like, which
  is to say that they have properties that correspond to those of, their real-
  world counterparts.
*)
...
(*
  The crucial concept that we introduce here is that of unification. It is the heart of
  pattern matching. Unification involves a pattern and a target term, and results in
  either failure to match, or a matching, where named elements of the pattern are 
  bound to elements of the target term. Those bound names can then be used in
  expressions within the scope of the pattern matching operation. 

  As an example, the pattern (S n') does not unify with the target term O, but it does
  unify with (S (S (S O))), with n' thereby being bound to (S (S O)). The predecessor
  function implementation thus uses pattern matching to "strip away one S from the
  non-zero argument to the function."
*)
...
(*
  Sullivan: Pattern matching can be pretty sophisticated. You can use it to "take apart" 
  complex terms into constitutent parts, which is often exactly what you need to do to
  compute a desired result by extracting, and somethings then combining, parts of 
  given terms.
*)
...
Theorem evenbEquiv: forall n: nat, evenb n = evenb' n.
induction n.
reflexivity.
simpl.
Admitted.
...
(*
  Sullivan: It is incredible important to understand how terms involving recursive
  functions get reduced. Consider three examples in which [evenb] is applied.

  [evenb O] matches the first rule and thus reduces to true
  
  [evenb (S O)] matches the second rule and thus reduces to false
  
  [evenb (S (S (S O)))] matches the third rule, with [n'] bound to [S O], and the 
  function applications thus reduces to whatever [evenb (S O)] reduces to! You
  have to keep evaluating until you reduce all the way to a basic value.
*)
...
(*
Sullivan: Evaluate (reduce) by hand the following term: [(plus (S (S O)) (S O))].
*)
...
(*
Sullivan: Is everyone ok with wildcards in pattern matching? 
Does everyone understand that Coq has hardwired special case for nat?
Evaluate by hand the terms [minus 4 2] and [minus 2 4].
*)
...
(*
Sullivan: Let's just talk briefly about notation. Here we introduce several infix
operators (whereas Coq functions use prefix notation) for binary functions. How
infix operators should be parsed is ambiguous unless you introduce precedence
and associativity rules. That's what these notion definitions are doing. And they
say that these rules apply only when these operators are seen in the context of
...
(*
Sullivan: Little exercise: write this without nested match constructs.
*)
...
(*
Sullivan: evaluate by hand the expressions [leb 3 2] and [leb 2 4].
*)
...
Theorem foo: forall A B: Prop, A -> B.
intro.
intro.
intro.
...
(*
Sullivan: Let's talk about intro/intros. To do this, we need to talk
about sequents. A set of assumptions and a set of goals is called
a sequent. Logical operations transform sequents. A proof is just
a sequence of logical operations that transforms a sequent into
one in which no goals remain to be proved. 

The intro and intros tactics transform sequents in which the 
current goal either starts with a [forall] or is an implication (an
if then statement). It transforms the sequent by eliminating the
[forall] or the premise of the implication from the goal and by
replacing it with a corresponding assumption. So rather than
[forall n: nat] at the start of a goal, you end up with [n: nat] as
an assumption. Read this as, "assume n is an arbitrary nat."
Similarly, if you have [A -> B] as a goal, which you can read as
a demand to prove that "if A is true then B must be true", [intro] 
replaces the [A ->] in the goal with the assumption, [a: A]. You
read this as "assume that you already have a proof, [a], that 
[A] is true." Your new goal is to prove that, in this context, [B]
must be true.
*)
...
(*
Sullivan: It's essential to understand here that these proofs are "going through"
easily because the form of the proposition exactly matches the computation rule
used in the definition of the function. The following examples show you that even
seemingly trivial syntactic mismatches defeat such a proof strategy. Coq does do
pattern matching and term reduction on its own, but it really can't do any kind of
more sophisticated reasoning. You have to guide it when it gets stuck. Coq is thus
a proof assistant, which is to say it often requires your assistance! You, really, are
the proof assistant to Coq! It's distinguished from fully automated theorem provers,
such as symbolic logic model checkers, such as Spin and Alloy, and satisfiability 
modulo theory checkers, such as Z3. The advantage of these checkers is that they
require no human intervention to find proofs. The disadvantage is that they give
you dramatically less expressive languages in which to write propositions. There
are thus many propositions that that true that can't even be expressed in Alloy or
Z3, that can be expressed in Coq, albeit at the cost of requiring human assistance.
This is a fundamental tradeoff. The really good news is that you can program into
Coq "decision procedures" for restricted kinds of problems, so in principal, you can
program Alloy or Z3 into Coq itself, and provide tactics that can be used when you
have a problem that can be expressed in the limited language for which that tactic
is designed. We'll see the Coq [omega] tactic in a bit, which can be used to fully
automatically produce proofs for arithmetic propositions involving only addition (to
including multiplication by constants, which as we've seen here unrolls to multiple
additions).
*)
...
(*
Let's discuss rewrite. It requires an equality in the context, and 
it rewrites the goal by substituting, in the goal, the term on one 
side of the equals sign for the term on the other side. The arrow
in the rewrite tactic application says whether to replace the left
term with the right (->) or the right with the left (->). Try it out.
*)
...
(*
Sullivan: The destruct tactic does case analysis on the available constructors!
It's similar to induction except that it doesn't hand us induction hypotheses to 
use in complete our proofs.
*)
