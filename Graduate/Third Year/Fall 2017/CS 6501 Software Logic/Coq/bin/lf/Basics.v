...
(* Q: Can [bool -> bool -> bool] also be read, "Given one input of type
   [bool], this function produces another function that takes in one
   input of type [bool] that produces an output of type [bool]." *)
...
Theorem negation_fn_applied_twice :
  ...
  forall (b: bool), f (f b) = b.
Proof.
  intros f x b. rewrite -> x. rewrite -> x. rewrite -> negb_involutive.
  reflexivity.
Qed.
(* FILL IN HERE *)
(** [] *)
...
Inductive bin : Type :=
  | Q : bin (* O for zero *)
  | D : bin -> bin (* D for double (twice a binary number) *)
  | D1 : bin -> bin. (* D1 for double plus 1 (one more than twice a
                        binary number) *)
...
Example test_bin_incr1 : bin_nat(incr Q) = 1.
Proof. reflexivity. Qed.
Example test_bin_incr2 : bin_nat(incr (D Q)) = 1.
Proof. reflexivity. Qed.
Example test_bin_incr3 : bin_nat(incr (D1 Q)) = 2.
Proof. reflexivity. Qed.
Example test_bin_incr4 : bin_nat(incr (D (D1 Q))) = 3.
Proof. reflexivity. Qed.
Example test_bin_incr5 : bin_nat(incr (D1 (D Q))) = 2.
Proof. reflexivity. Qed.
(* FILL IN HERE *)
(** [] *)
