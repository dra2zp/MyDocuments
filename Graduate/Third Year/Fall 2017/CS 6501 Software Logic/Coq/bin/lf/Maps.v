...
Lemma t_update_eq : forall A (m: total_map A) x v,
  (t_update m x v) x = v.
Proof. intros A m x v. unfold t_update. rewrite <- beq_id_refl.
  reflexivity. Qed.
...
Proof. intros X v x1 x2 m H. unfold t_update. apply false_beq_id in H.
...
Lemma t_update_shadow : forall A (m: total_map A) v1 v2 x,
  ...
  = t_update m x v2.
Proof. intros A m v1 v2 x. apply functional_extensionality. intros x'.
  unfold t_update. destruct (beq_id x x'). reflexivity. reflexivity. Qed.
...
Proof. intros x y. apply iff_reflect. rewrite beq_id_true_iff. reflexivity.
  Qed.
...
Theorem t_update_same : forall X x (m : total_map X),
  t_update m x (m x) = m.
Proof. intros X x m. apply functional_extensionality. intros x0.
  unfold t_update. destruct (beq_id x x0) eqn: H.
...
Proof. intros X v1 v2 x1 x2 m Hneq. apply functional_extensionality.
