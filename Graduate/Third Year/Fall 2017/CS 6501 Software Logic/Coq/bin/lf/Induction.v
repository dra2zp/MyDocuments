...
(** Briefly explain the difference between the tactics [destruct]
    and [induction].
(* Destruct is used when there are a finite number of cases to be proven,
   whereas induction is used when something must be proven for an
...
(** Translate your solution for [plus_comm] into an informal proof:
    ...
    Proof: We want to prove that a + (b + c) = (b + c) + a. Suppose that
           a, b, and c are natural numbers. For a = b = c = 0, we have
           that 0 + (0 + 0) = (0 + 0) + 0 => 0 = 0. Now, suppose that
           (a + n) + ((b + n) + (c + n)) = ((b + n) + (c + n)) + (a + n).
           (a + (n + 1) + ((b + (n + 1) + (c + (n + 1))) = ((b + (n + 1) +
           ) + (c + (n + 1))) + (a + (n + 1)). From the definition of +,
           and using our induction hypothesis, this follows.
           (* FILL IN HERE *)
*)
(** [] *)
...
Proof.
  ...
  - rewrite <- mult_n_Sm. simpl. rewrite -> IHm. rewrite -> plus_comm. 
    reflexivity.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Theorem plus_swap' : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros m n p. rewrite -> plus_assoc. rewrite -> plus_assoc.
...
Theorem bin_to_nat_pres_incr : forall b : bin,
  bin_nat (incr b) = S (bin_nat b).
Proof.
  intros b. induction b.
  - reflexivity.
  - simpl. rewrite <- plus_n_O. Admitted.
(* INCORRECT *)
(* FILL IN HERE *)
(** [] *)
