...
Theorem ev_8 : ev 8.
Proof. apply ev_SS. apply ev_SS. apply ev_SS. apply ev_SS. apply ev_0.
  Qed.
...
Proof. intro. apply (plus H H). Qed.

Print my_fun.

(* Definition my_fun' : nat -> bool.
Proof. intro. refine (if (_ : bool) then _ else _).
  - exact (beq_nat H). Qed. *)
...
Definition foobar: ex ev.
Proof. apply ex_intro with (x := 0). apply ev_0. Qed.
...
Theorem bar: 1 + 1 = 2.
Proof. simpl. apply eq_refl with (x := 2). Qed.
...
Proof. intros x y eq1 eq2 eq3 eq4. destruct eq2. apply eq4. Qed.
