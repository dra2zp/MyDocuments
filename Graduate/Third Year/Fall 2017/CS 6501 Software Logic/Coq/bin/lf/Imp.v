...
Definition aexp1 := APlus (ANum 3) (ANum 5).
Definition aexp2 := AMult (ANum 4) (aexp1).
...
Theorem ar1: aevalR (ANum 3) 3.
Proof. apply E_ANum. Qed.

Theorem ar2: aevalR (ANum 3) 4.
Proof. Abort.
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BEq : forall a1 a2 n1 n2, (a1 \\ n1) -> (a2 \\ n2) ->
    bevalR (BEq a1 a2) (beq_nat n1 n2)
  | E_BLe : forall a1 a2 n1 n2, (a1 \\ n1) -> (a2 \\ n2) ->
    bevalR (BLe a1 a2) (leb n1 n2)
  | E_BNot : forall b b', bevalR b b' -> bevalR (BNot b) (negb b')
  | E_BAnd : forall b1 b2 b1' b2', (bevalR b1 b1') -> (bevalR b2 b2') ->
...
Proof. split.
  - intros H. induction H.
    ...
    + simpl. rewrite -> aeval_iff_aevalR in H.
      rewrite -> aeval_iff_aevalR in H0. rewrite -> H. rewrite -> H0.
      reflexivity.
    + simpl. rewrite -> aeval_iff_aevalR in H.
      rewrite -> aeval_iff_aevalR in H0. rewrite -> H. rewrite -> H0.
      reflexivity.
  ...
  - generalize dependent bv. induction b.
    ...
    + simpl. intros. rewrite <- H. apply E_BEq. rewrite -> aeval_iff_aevalR.
      reflexivity. rewrite -> aeval_iff_aevalR. reflexivity.
    + simpl. intros. rewrite <- H. apply E_BLe. rewrite -> aeval_iff_aevalR.
      reflexivity. rewrite -> aeval_iff_aevalR. reflexivity.
    + simpl. intros. rewrite <- H. apply E_BNot. apply IHb. reflexivity.
    + simpl. intros. rewrite <- H. apply E_BAnd. apply IHb1. reflexivity.
...
Example easyone: forall st, ceval SKIP st st.
Proof. apply (E_Skip). Qed.

Check empty_state.

Example lesseasy: ceval (X ::= (APlus (ANum 2) (ANum 3)))
  empty_state (t_update empty_state X 5).
Proof. apply (E_Ass). simpl. reflexivity. Qed.
...
Proof. apply E_Seq with (t_update empty_state X 0).
  - apply E_Ass. reflexivity.
  - apply E_Seq with (t_update (t_update empty_state X 0) Y 1); apply E_Ass;
    reflexivity.
Qed.
...
Proof. unfold pup_to_n.
  apply E_Seq with (t_update (t_update empty_state X 2) Y 0). apply E_Ass.
  reflexivity.
  apply E_WhileTrue with (t_update (t_update (t_update
    (t_update empty_state X 2) Y 0) Y 2) X 1). reflexivity.
  apply E_Seq with (t_update (t_update (t_update empty_state X 2) Y 0) Y 2).
  apply E_Ass. reflexivity. apply E_Ass. reflexivity.
  apply E_WhileTrue with (t_update (t_update (t_update (t_update (t_update
    (t_update empty_state X 2) Y 0) Y 2) X 1) Y 3) X 0). reflexivity.
  apply E_Seq with (t_update (t_update (t_update (t_update
    (t_update empty_state X 2) Y 0) Y 2) X 1) Y 3). apply E_Ass. reflexivity.
  apply E_Ass. reflexivity. apply E_WhileFalse. reflexivity.
Qed.
...
Theorem XtimesYinZ_spec : forall st n m st',
  st X = m -> st Y = n -> XtimesYinZ / st \\ st' -> st' Z = m * n.

Proof. intros st n m st' HX HY He. inversion He. subst. clear He. simpl.
  apply t_update_eq.
Qed.
...
Proof. intro c. split.
  - intro H. induction c; try constructor; simpl in H;
    try (rewrite -> andb_true_iff in H; destruct H as [H1 H2];
    try (apply IHc1; apply H1); try (apply IHc2; apply H2));
    try inversion H.
