...
(*
values (proofs) of these types are recursively constructed.
*)

Theorem zero_is_ev: ev 0.
Proof. exact ev_0. Qed.

Theorem two_is_ev: ev 2.
Proof. exact (ev_SS 0 ev_0). Qed.

Theorem four_is_ev: ev 4.
Proof. exact (ev_SS 2 (ev_SS 0 ev_0)). Qed.

Theorem four_is_ev': ev 4.
Proof. exact (ev_SS 2 two_is_ev). Qed.
...
Theorem ev_double : forall n,
  ev (double n).
Proof. intros n. rewrite double_plus. induction n as [| n'].
  - simpl. apply ev_0.
...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof. intros n E. inversion E as [|n' E' H'].
  inversion E' as [|n'' E'' H'']. apply E''. Qed.

Theorem even5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof. intros E. inversion E as [|n' E' H'].
  inversion E' as [|n'' E'' H'']. inversion E''. Qed.
...
Proof. intros n m En Em. induction En as [|n' En'].
  - simpl. apply Em.
...
Theorem ev_ev__ev : forall n m,
  ev (n+m) -> ev n -> ev m.
Proof. intros n m H0 H1. induction H1.
  - simpl in H0. apply H0.
  - apply IHev. assert (S (S n) + m = S (S (n + m))) as H.
    { reflexivity. }
    rewrite H in H0. apply evSS_ev in H0. apply H0.
Qed.
...
Proof. intros n m p Hnm Hnp.
  ...
  apply H in Hnm. apply (ev_ev__ev (n + p)). apply Hnm. apply Hnp.
Qed.
...
Inductive total_relation : nat -> nat -> Prop :=
  | mk_total_proof : forall n m : nat, total_relation n m.

Example three_four: total_relation 3 4.
Proof. apply mk_total_proof. Qed.
...
(*
The following theorems give us (and illustrate) rewrite
rules that often prove useful. E.g., how do we eliminate the S's
on both sides in [S n <= S m] when such a proposition appears
either as a goal or as an assumption/hypothesis? E.g., how do
we formalize, and reason both forwards and backwards, using
the transitivity of le?
*)
...
Proof. intros m n o H1 H2.
(* H2 is either le_n with o = n; or le_S, with o = S o' and le n o' *)
induction H2 as [|o'].
(* if o = n, we're done; m <= o is m <= n, true by assumption *)
  - apply H1.
(* otherwise throw away one S and use the induction hypothesis *)
  - apply le_S. apply IHle.
Qed.
(*
Proof. intros m n o eq1 eq2. rewrite eq1. rewrite eq2. apply le_n. Qed.
*)
...
Theorem le_plus_l : forall a b,
  a <= a + b.
Proof. intros a b. induction a as [| a'].
  - apply O_le_n.
...
Theorem plus_lt : forall n1 n2 m,
  n1 + n2 < m ->
  n1 < m /\ n2 < m.
Proof. unfold lt. intros n1 n2 m H. split.
...
Theorem lt_S : forall n m,
  n < m ->
  n < S m.
Proof. unfold lt. intros n m H. apply le_S. apply H. Qed.

Theorem leb_complete : forall n m,
  leb n m = true -> n <= m.
(* induction on m, generalize dependent n, don't over-intro *)
Proof. intros n m H. generalize dependent n. induction m.
...
Theorem leb_correct : forall n m,
  n <= m ->
  leb n m = true.
Proof. intros n m H. generalize dependent n. induction m. intros n H.
  - inversion H. rewrite <- leb_refl. reflexivity.
  - intros n H. inversion H.
    + rewrite <- leb_refl. reflexivity.
    + apply IHm in H1. Admitted.

(** Hint: This theorem can easily be proved without using [induction]. *)

Theorem leb_true_trans : forall n m o,
  leb n m = true -> leb m o = true -> leb n o = true.
Proof. intros n m o Hnm Hmo. apply leb_correct. apply leb_complete in Hnm.
  apply leb_complete in Hmo. apply (le_trans n m). apply Hnm. apply Hmo.
  Qed.
...
(*Inductive subseq: list A -> list A -> Prop :=
  | nil: forall l, subseq [] l
  | skip: forall l1 l2 : list A, a: A,
    subseq l1 l2 -> subseq l1 (a :: l2)
  | third: -> subseq (a :: l1) (a :: l2).
*)
...
(*
Regular expressions are really a simple language for
describing *sets* of strings over some alphabet, i.e., "languages."
There is thus a language of regular expressions, and then we will
interpret each sentence in this RE language as specifying another
set of string over that same alphabet.

The language of regular expressions (over an alphabet) has its
own syntax. We then interpret each string in this RE language, i.e.,
each regular expression, as "meaning" another language, i.e., the
set of strings that "matches" the given regular expression. We can
call the association of regular expressions to their corresponding
meanings the *semantics* of the RE language.

So, before we can do much with RE's, we need to specify both
their syntax and their semantics. In this section we start by defining
a data type that specifies the *syntax* of regular expressions. Each
value of this type will be (we will interpret it as) a RE. A key idea is
that the constructors of this data type directly reflect the recursive
definition of the syntax.

We use polymorphism to parameterize our [reg_exp] data type by the
alphabet. And, again, the constructors just implement the production
rules for the language of REs (over any given alphabet).
*)
...
(*
Now we specify the semantics of REs. We represent
the semantics as a binary *relation* in Coq over strings (which we
represent as lists of characters from the given alphabet) and REs.
That is, we represent the semantics of REs as a family of propositions,
[exp_match], indexed by strings and regular expressions, with proofs
for all and only those propositions, [exp_match str re], where we want
the given string, [str] to be in the language specified by the given [re].

We introduce a notation [str =~ re] as a shorthand for the proposition
that [str] is in the language defined by (or "matches with") [re]. Such
a proposition might or might not be provable, of course. The ones that
are provable pick out the elements of the desired relation.
*)
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp),
  ...
  fold app ss [] =~ Star re.
Proof. intros T ss re H. induction ss as [| s' ss'].
  - simpl. apply MStar0.
...
Proof. intros T s1 s2. split.
  - generalize dependent s1. induction s2 as [|x s2' IH].
    + intros s4 H. simpl in H. inversion H. reflexivity.
    + intros s4 H. simpl in H. inversion H. apply IH in H4. rewrite -> H4.
      inversion H3. simpl. reflexivity.
  - generalize dependent s2. induction s1 as [|x s1' IH].
    + intros s4 H. rewrite <- H. simpl. apply MEmpty.
    + intros s4 H. rewrite <- H. simpl.
      assert (HApp: x :: s1' = [x] ++ s1').
      { simpl. reflexivity. }
      rewrite -> HApp. apply MApp. apply MChar. apply IH. reflexivity.
Qed.
...
Proof. intros T re. split.
  - induction re.
    ...
    + intros [s H]. inversion H. simpl.
      ...
      rewrite -> Hre1. assert (Hre2: re_not_empty re2 = true).
      { apply IHre2. exists s2. apply H4. }
      rewrite -> Hre2. reflexivity.
  ...
  - induction re.
    ...
    + intro H. simpl in H. destruct (re_not_empty re1).
      { destruct (re_not_empty re2). assert (H1: true = true).
        { reflexivity. }
        assert (H2: true = true).
        { reflexivity. }
        apply IHre1 in H1. apply IHre2 in H2. destruct H1 as [s1 H1].
        destruct H2 as [s2 H2]. exists (s1 ++ s2). apply MApp. apply H1.
        apply H2. simpl in H. inversion H. }
      simpl in H. inversion H.
    + intros H. simpl in H.
      destruct (re_not_empty re1). simpl in H. apply IHre1 in H.
      destruct H as [s H]. exists s. apply MUnionL. apply H.
      { destruct (re_not_empty re2). simpl in H. apply IHre2 in H.
        destruct H as [s H]. exists s. apply MUnionR. apply H. simpl in H.
        inversion H. }
...
Lemma MStar'' : forall T (s : list T) (re : reg_exp),
  ...
  exists ss : list (list T),
    s = fold app ss []
    /\ forall s', In s' ss -> s' =~ re.
Proof. intros T s re H. remember (Star re) as re'. induction H.
  - inversion Heqre'.
  - inversion Heqre'.
  - inversion Heqre'.
  - exists []. simpl. split.
    + inversion H. reflexivity. rewrite -> H0. Admitted.
...
Definition no_two_nums_in_a_row {X} (l : list X) (x : X) : Prop :=
  match l with
  | [] => True
  | h :: t => h <> x
  end.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  ...
  | nostutterCons : forall l x, nostutter l /\ no_two_nums_in_a_row l x ->
    nostutter (x :: l).
...
Example test_nostutter_4:      not (nostutter [3;1;1;4]).
  ...
  Proof. intro. inversion H. destruct H1 as [H1 _]. inversion H1.
    destruct H4 as [_ H4]. simpl in H4. apply H4. reflexivity. Qed.
...
Proof. intros. induction l as [|x' l'].
  - simpl. inversion H.
  - simpl. destruct H as [H0 | H1]. exists []. simpl. rewrite H0.
    exists l'. reflexivity. apply IHl' in H1. destruct H1 as [l1 [l2 H]].
...
Inductive repeats {X:Type} : list X -> Prop :=
  | repeatsNil : forall l x, repeats l -> repeats (x :: l)
...
Theorem pigeonhole_principle: forall (X:Type) (l1  l2:list X),
   ...
   length l2 < length l1 ->
   repeats l1.
Proof. intros X l1. induction l1 as [|x l1' IHl1']. Admitted.
