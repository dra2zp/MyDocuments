...
Proof.
  intros A B PfA H. apply H. apply PfA.
Qed.
...
(* Theorem apply2: forall A B: nat -> Prop, forall n: nat,
  A 3 -> (A n -> B n) -> B 3.
Proof.
  intros A B n PfA H. apply H.
Abort. *)
(* This doesn't work because applying H doesn't bind with the
conclusion since forall is different *)
(* Must have universally quantified variable in hypothesis in order
to use apply. *)
...
Proof.
  intros. apply f_equal. apply H. (* assumption and trivial can be used
                                     instead of apply H. *)
Qed.
...
Proof.
  ...
  - intros l1 l2 H. destruct p. simpl in H. destruct (split l').
  simpl in H. inversion H. simpl. rewrite -> IHl'. reflexivity. reflexivity.
Qed.

(* READ THIS WHEN FEELING FRUSTRATED: *)
(* "Have you ever looked at it and wanted to run away? Don't run away." -KS *)
  (* FILL IN HERE *)
(** [] *)
