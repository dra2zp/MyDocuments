...
Theorem double_neg_sullivan: forall P: Prop, ~~P -> P.
Proof. intros. unfold not in H. Abort.
...
Proof. intros X. unfold tr_rev. unfold rev. unfold rev_append. simpl.
  Admitted.
(* HELP *)
...
Proof.
  ...
  - destruct (evenb n') eqn: Heq.
    ...
    + rewrite evenb_S. rewrite Heq. simpl. destruct IHn' as [n'' IHn'].
      exists (n'' + 1). rewrite IHn'. rewrite double_plus.
      rewrite double_plus. rewrite plus_n_Sm. rewrite <- plus_1_l.
      rewrite <- plus_n_Sm. rewrite <- (plus_1_l (n'' + n'')).
...
Proof. intros x y. unfold not. split.
  ...
  - intros H. induction x as [| x'].
    ...
    + induction y as [| y'].
      { generalize dependent x'. reflexivity. }
      { simpl. destruct (beq_nat x' y') eqn: Heq.
        - exfalso. apply H. apply f_equal. apply beq_nat_true_iff.
          apply Heq.
        - reflexivity. }
Qed.
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
  ...
  | _, [] => false
  | [], _ => false
  | h1 :: t1, h2 :: t2 => if (beq h1 h2) then beq_list beq t1 t2 else
    false
  end.
...
Proof. intros A beq H l1 l2. split.
  ...
  - generalize dependent l2. induction l1 as [| h1 l1' IHl1'].
    ...
    + induction l2 as [| h2 l2' IHl2'].
      ...
      { simpl. intros H1. destruct (beq h1 h2) eqn: Heq.
        ...
        - inversion H1. apply H in H2. rewrite H2 in Heq. symmetry.
          apply Heq. }
Qed.
...
Proof. intros. split.
  - intros. induction l.
    + simpl. reflexivity.
    + simpl. split.
      { unfold forallb in H. rewrite andb_true_iff in H.
        apply proj1 in H. apply H. }
      { simpl in H. apply IHl. inversion H. rewrite andb_true_iff in H1.
        destruct H1 as [H2 H3]. rewrite H2. rewrite H3. simpl.
        reflexivity. }
  - intros. induction l.
    + reflexivity.
    + simpl. rewrite andb_true_iff. split.
      { simpl in H. apply proj1 in H. apply H. }
      { simpl in H. apply proj2 in H. apply IHl in H. apply H. }
Qed.
...
Theorem excluded_middle_irrefutable:  forall (P:Prop),
  ~ ~ (P \/ ~ P).
Proof. unfold not. intros P H. apply H. right. intros. apply H. left.
  apply H0. Qed.
...
Proof. unfold excluded_middle. intros ex X f H x. unfold not in H.
  unfold not in ex. destruct (ex (f x)) as [H1 | H2].
  - apply H1.
  - destruct H.
    + exists x. apply H2.
Qed.
