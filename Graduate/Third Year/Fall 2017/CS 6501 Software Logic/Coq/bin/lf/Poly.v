...
Lemma map_associativity: forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite -> map_associativity. rewrite -> IHl. simpl. reflexivity.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Definition foo (n1 n2 n3 n4: nat) :=
  n1 + n2 + n3 + n4.

Check foo.

Definition foo' := (mult 3).
Check foo'.
Compute foo'.

Definition mapS := (map S).
Check mapS.
Compute mapS [1;2;3].
Definition mapfoo' := map foo'.
Compute mapfoo' [1;2;3].
...
Proof. (* FILL IN HERE *)
  unfold plus. unfold two. unfold three. unfold doit3times. reflexivity.
Qed.
